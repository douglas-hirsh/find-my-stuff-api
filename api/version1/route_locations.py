...
from db.repository.location import create_new_location, delete_location, get_all_locations, get_location_by_id, update_location
...
@router.delete("/{id}", response_model=ShowLocation)
async def update_locations(id: int, db: Session = Depends(get_db), current_user:User = Depends(get_current_user_from_token)):
    existing_location = get_location_by_id(id, db, current_user)

    if not existing_location:
        raise HTTPException(status_code=404, detail="Location not found")

    return delete_location(existing_location, db)